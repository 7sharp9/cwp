...
let ``the followed-path cache is outside the canonical image and the format version is unaffected by it`` () =
    ...
    // (Canonical.FormatVersion is 4 as of TASK-028 — Progress (TASK-018),
    // TacticalKnowledge (TASK-026), Order / Disposition (TASK-028), not Route —
    // but nothing about those bumps is exercised by this test.)
