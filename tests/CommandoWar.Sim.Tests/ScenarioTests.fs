...
let ``the content version is independent of the canonical and replay versions`` () =
    ...
    // is 2 (TASK-010 authored terrain layer), Canonical.FormatVersion is 6
    // (TASK-018 / TASK-026 / TASK-028 / TASK-032 / TASK-033), Replay.FormatVersion
    // is 1. This test documents the intent, not an inequality.
...
let ``the six-agent fixture as a Scenario reproduces the pinned initial hash`` () =
    ...
    | Error e -> Assert.Fail($"World.ofScenario failed: {e}")
    | Ok world -> Assert.Equal(0xF1A703A752C0F6B9UL, (Hashing.hash world).Value)
...
let ``the fixture Scenario stepped 40 ticks with the fixture command reaches the pinned final hash`` () =
    ...
    Assert.Equal(0x507D041E109404B6UL, (Hashing.hash state).Value)
