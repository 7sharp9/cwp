...
| `formation-slots` | **The first entry authoring a formation** (TASK-059, backlog B-011d). Two friendly agents in a two-slot "wedge" formation (offsets `(-1,0)`/`(1,0)`), both ordered to the identical nominal cell `(5,5)` on tick 1: each resolves its own real `MoveTo` destination as that shared anchor plus its own authored slot offset (`Appraisal.resolveFormationTarget`), landing on `(4,5)` and `(6,5)` instead of colliding on `(5,5)`. |
