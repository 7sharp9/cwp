...
| R-003 | Mibo changes rapidly or exposes correctness defects | 3 | 4 | breaking upgrade, stale-read issue, adaptive graph inconsistency | pin exact version; use classic MVU only; keep simulation independent | replace Mibo host with direct backend host or Godot | closed |
| R-004 | Godot C# to F# boundary creates friction and debugging opacity | 3 | 3 | excessive DTOs, reflection, hot-reload failures, stack traces crossing layers | one thin facade, plain .NET types, no F# types exposed to Godot serialization; dedicated low-impedance boundary-design task before P4 client work | keep C# host layer minimal and push logic into F#; if friction is severe, revisit at G4 with measured evidence | mitigating |
